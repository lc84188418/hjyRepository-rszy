...
@RestController
@Slf4j
@DefaultProperties(defaultFallback ="consumer_user_global_fallback")
public class HystrixUserController {
    ...
    @HystrixCommand(fallbackMethod = "consumer_fallback_consumerSystemUserAddPage",commandProperties = {
            @HystrixProperty(name = "execution.isolation.thread.timeoutInMilliseconds",value = "20000")
    })
    @GetMapping(value = "/consumer/system/user/addPage")
    public CommonResult systemUserInsertPage(){
        return hystrixUserService.systemUserInsertPage();
    }
    @GetMapping(value = "/consumer/system/user/add")
    public Map<String,Object> systemUserInsertAdd(){
        return hystrixUserService.systemUserInsertAdd();
    }
    @PostMapping(value = "/consumer/system/user/list")
    public CommonResult systemUserInsertList(@RequestBody String param){
        return hystrixUserService.systemUserInsertList(param);
    }

        //当某服务（consumerSystemUserList）失败，就会调用兜底的fallback方法
    public CommonResult consumer_fallback_consumerSystemUserAddPage() {
        return new CommonResult(444,"error","serverPort:"+serverPort+",我是消费者8083，用户系统繁忙，请稍后再试o(╥﹏╥)o",null);
    }
    /**
     *
     * @return
...
//    }
    //下面是全局fallback
    public CommonResult consumer_user_global_fallback() {
        return new CommonResult(444,"error","serverPort:"+serverPort+",我是消费者8083,user_global全局异常处理信息o(╥﹏╥)o",null);
