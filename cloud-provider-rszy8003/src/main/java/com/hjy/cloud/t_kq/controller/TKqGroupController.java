 ...
 */
//作用在模块API类上，对API模块进行说明，如果没有该注解，默认为t-kq-group-controller
@Api(tags = "考勤分组")
@RestController
public class TKqGroupController {
     ...
     * 1 跳转到新增页面
     */
    @ApiOperation("添加考勤分组页面接口")//作用在API方法上，对操作进行说明
     ...
     * @return 新增结果
     */
    @ApiOperation("添加考勤分组接口")
    @PostMapping(value = "/kq/group/add")
    public CommonResult insert(@ApiParam(name = "JSON参数", required = true)@RequestBody String param) throws FebsException {
        try {
     ...
     * @return 修改结果
     */
    @PostMapping(value = "/kq/group/update")
